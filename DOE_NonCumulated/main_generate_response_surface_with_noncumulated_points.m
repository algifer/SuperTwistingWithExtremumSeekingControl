...
%% Non-Cumulated Model (30 points - best model)
non_cumulated_model = true;
filename_doe = 'IO_regression_By_DOE_with_Noise_30_main_points_noncumulated.xlsx';
filename_trained = 'trainedModel_with_30_sample_best_selected_noncumulated.mat';
...
subplot(2, 1, 1);
plot(GluInLoad, QinLoad, 'o', 'LineWidth', 2, 'MarkerSize', 6); % markerSize mejora visibilidad
axis([0 30 0 6]);
...
for j=1:lQin
    for k=1:lGluIn
        ...
        Hpr_fit(j,k) = trainedModelWithNoise.predictFcn(TI); % Non-Cumulated Points
    end
end
